...
function Workouts() {
  ...
  const content = useMemo(() => {
    ...
    if (workouts.length === 0) {
      return <p className="status-text">No workouts available.</p>
    }
          ...
          <tbody>
            {workouts.map((workout, index) => (
              <tr key={workout._id ?? `${workout.title}-${index}`}>
                <td>{workout.title ?? workout.name ?? 'n/a'}</td>
                <td>{workout.level ?? 'n/a'}</td>
                <td>{workout.minutes ?? workout.duration ?? 'n/a'}</td>
                <td>{workout.focus ?? workout.type ?? 'n/a'}</td>
              </tr>
            ))}
          </tbody>
        </table>

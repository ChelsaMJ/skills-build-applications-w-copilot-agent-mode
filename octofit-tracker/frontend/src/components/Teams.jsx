...
const API_ENDPOINT = import.meta.env.VITE_CODESPACE_NAME
  ...
  : browserHostname.endsWith('.app.github.dev')
    ? `https://${browserHostname.replace(/-\d+\.app\.github\.dev$/, '-8000.app.github.dev')}/api/teams/`
...
function Teams() {
  ...
  const content = useMemo(() => {
    ...
    if (teams.length === 0) {
      return <p className="status-text">No teams available.</p>
    }
          ...
          <tbody>
            {teams.map((team, index) => (
              <tr key={team._id ?? `${team.name}-${index}`}>
                <td>{team.name ?? 'n/a'}</td>
                <td>{team.captainId ?? team.captain ?? 'n/a'}</td>
                <td>{team.city ?? 'n/a'}</td>
              </tr>
            ))}
          </tbody>
    ...
    <section className="panel-card card border-0 shadow-sm">
      <div className="card-body p-4">
        <div className="panel-head mb-3">
          ...
          <small className="text-muted">Records shown: {teams.length} / {total}</small>
        </div>
        {content}
      </div>
    </section>
